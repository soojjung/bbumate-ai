...
OpenAI 임베딩 모델을 초기화하고 제공하는 공통 유틸리티입니다.
"""
...
def get_embeddings() -> OpenAIEmbeddings:
    """OpenAI 임베딩 객체를 생성하여 반환.

    환경 변수에서 API 키와 모델 이름을 자동으로 로드합니다.

    Returns:
        OpenAIEmbeddings: 초기화된 임베딩 모델
    ...
    try:
        ...
        if not settings.OPENAI_EMBEDDING_MODEL:
            raise ConfigurationError(
                "OPENAI_EMBEDDING_MODEL이 설정되지 않았습니다. .env 파일을 확인하세요."
            )

        logger.info(f"OpenAI 임베딩 모델 '{settings.OPENAI_EMBEDDING_MODEL}' 초기화")
